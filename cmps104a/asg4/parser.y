...
structdef : structh '}' { $$ = $1; free_ast($2); }
          | structh strdecl ';' '}' { $$ = stealkids ($1, $2); 
                                      free_ast2($3, $4); }
          ;

structh : TOK_STRUCT typedecl '{' { $$ = adopt1sym ($1, $2, 
          TOK_STRUCT); free_ast ($3); }
        ;
      
typedecl : TOK_IDENT { $$ = adoptsym($1, TOK_TYPEID); }
         ;

strdecl : strdecl ';' fielddecl { $$ = adopt1 ($1, $3); free_ast ($2); }
        | fielddecl { $$ = adopt1(new_structure($1), $1); } 
        ;
        
fielddecl : type fielddeclid { $$ = adopt1 ($1, $2) }
          ;

fielddeclid : TOK_IDENT { $$ = adoptsym ($1, TOK_FIELD); }
            ;

identdecl : type identdeclid { $$ = adopt1 ($1, $2) }
          ;

identdeclid : TOK_IDENT { $$ = adoptsym ($1, TOK_DECLID); }
            ;
...
typeid : TOK_IDENT      { $$ = adoptsym($1, TOK_TYPEID); }
       ;

params : params ',' identdecl { adopt1sym($1, $3, TOK_PARAMLIST);
                              free_ast($2); }
...
allocator : TOK_NEW typeid '(' ')' { $$ = adopt1($1, $2);
                                       free_ast2 ($3, $4); }  
...
call : callfront ')' { $$ = $1; free_ast($2); }
     | callfront exprs ')' { free_ast ($3); $$ = stealkids($1, $2); }
     ;
            
variable : TOK_IDENT           { $$ = $1; }
         | expr '[' expr ']' %prec '['  { $$ = adopt1sym ($1, $3, 
                                TOK_VARDECL); free_ast2 ($2, $4); }
         | expr '.' fielddeclid { $$ = adopt2sym ($2, $1, $3, 
                                 TOK_VARDECL); }
         ;            
